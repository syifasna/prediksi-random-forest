      ...
      <div class="card-body px-0 pt-0 pb-2">
        <div class="table-responsive p-0">
            <div class="container">
                <table class="table mt-4" style="text-align: center">
                    ...
                    <tbody>
                    @forelse ($semester as $kt)
                        <tr>
                            <td>{{ ++$i }}</td>
                            <td>{{ $kt->tahun_ajaran->ket }}</td>
                            <td>{{ $kt->semester }}</td>
                            <td>
                                <form action="{{ route('semester.destroy',$kt->id) }}" method="POST">
                                    <a class="btn btn-warning btn-xs" href="{{ route('semester.edit',$kt->id) }}"><ion-icon name="create" style="font-size: 15px;"></ion-icon></a>
                                    @csrf
                                    @method('DELETE')
        ...
        <div>
            <p class="mb-0 text-sm text-muted">
                Menampilkan {{ $semester->firstItem() }} - {{ $semester->lastItem() }} dari {{ $semester->total() }} data
            </p>
        </div>
        <div>
            {{ $semester->onEachSide(1)->links('pagination::bootstrap-4') }}
        </div>
    </div>
        ...
        <div class="card carddar mb-4">
            ...
            <div class="card-body px-0 pt-0 pb-2">
                                        ...
                                        <td>
                                            <form action="{{ route('semester.destroy', $kt->id) }}" method="POST">
                                                <a class="btn btn-warning btn-xs"
                                                    href="{{ route('semester.edit', $kt->id) }}"><ion-icon name="create"
                ...
                <div class="d-flex justify-content-between align-items-center px-4 mt-3">
                    <div>
                        <p class="mb-0 text-sm text-muted">
                            Menampilkan {{ $semester->firstItem() }} - {{ $semester->lastItem() }} dari
                            {{ $semester->total() }} data
                        </p>
                    </div>
                    <div>
